#!/usr/bin/env python3
"""Select E6 alignment lambda from validation histories only."""
...
def main() -> int:
    ...
    payload = select_alignment_candidate(
        experiment_family=args.experiment_family,
        candidate_roots=roots,
        output_path=output,
    )
    print(
        f"alignment_candidate_selected=True family={payload['experiment_family']} "
        f"lambda={payload['selected_lambda']} eer={payload['selected_validation_eer']}"
    )
    return 0
